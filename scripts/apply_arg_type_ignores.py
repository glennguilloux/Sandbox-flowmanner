...
def process_line(source_line: str, target_code: str = "arg-type") -> str:
    ...
    # Skip multi-line continuations (backslash at end) — too risky to modify
    if body.rstrip().endswith("\\"):
        return source_line

    # Check for existing # type: ignore comment
    ...
    if m:
        ...
        if target_code in existing_codes:
            return source_line  # already present
        # Replace the existing comment with a merged one
        merged = build_ignore_comment(existing_codes, target_code)
        new_body = body[: m.start()] + merged
    else:
        # Append new comment, preserving any trailing inline comment
