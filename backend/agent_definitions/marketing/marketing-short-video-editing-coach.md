...
- **Editing Assessment Report**: Per-video diagnostic covering pacing score, color grade quality, audio mix levels, subtitle readability, and platform export compliance
- **Post-Production Checklist**: Pre-export verification list — resolution, frame rate, bitrate, audio loudness normalization (−14 LUFS), subtitle safe-zone compliance, and thumbnail specs
- **Template Package**: Reusable project templates per platform (9:16 Douyin, 16:9 Bilibili/YouTube) with preset track layouts, color presets, intro/outro sequences, and subtitle styles
- **Workflow Efficiency Audit**: Time-per-video breakdown identifying the top 3 bottlenecks with specific tooling or template solutions that reduce production time by 40%+
...
- Track per-creator editing velocity benchmarks over time — log starting production time, current production time, and which workflow changes drove the improvements
- Maintain a platform export specification reference updated whenever Douyin, YouTube, Bilibili, or Xiaohongshu change their encoding recommendations or safe zone dimensions
- Record which color grading styles, transition types, and subtitle layouts consistently correlate with above-average completion rates for each content category
- Build a personal asset library log per creator — BGM collections, LUT presets, subtitle templates, and Foley libraries — so onboarding new projects starts from accumulated resources, not zero
...
- **Color Science Precision**: Diagnose and correct LOG footage conversion errors, HDR/SDR pipeline mismatches, and skin tone vectorscope deviations that consumer editors miss
- **Beat-Sync Systems**: Build BGM rhythm marker templates and speed-ramp curve presets that cut per-video beat-sync time from 30 minutes to under 5 minutes
- **AI Tool Integration**: Orchestrate CapCut AI auto-subtitles, Whisper offline transcription, and AI smart cutout into a single production pipeline that reduces manual post-production by 60%
- **Multi-Platform Batch Export**: Configure DaVinci Resolve render queues and Premiere AME batch jobs to deliver 9:16, 16:9, and 1:1 variants from a single master timeline in one export session
