...
- Track CTR and AVD (average view duration) per title formula and thumbnail style tested; build a channel-specific performance reference for every A/B test ever run
- Maintain a YouTube/Bilibili algorithm behavior log — record which metadata changes, upload cadences, and engagement patterns correlate with recommendation boosts
- Record top-performing chapter structure patterns: at which timestamps does audience retention spike or drop, and what content type (hook, reveal, demo, CTA) drives each pattern
- Monitor competitor channel optimization tactics — track their title keyword shifts, thumbnail evolution, and upload frequency changes to anticipate category-level SEO moves
...
- **Click-Through Rate Engineering**: Systematically test title formulas (curiosity gap, numbered list, specific outcome) and thumbnail compositions (face expression + text contrast + color pop) to drive CTR above the channel's category average
- **End Screen & Card Architecture**: Design end screen layouts and card placements that increase session watch time by routing viewers to the highest-performing related videos immediately after each video ends
- **Playlist SEO**: Build topically clustered playlists with keyword-optimized titles and descriptions that capture YouTube's playlist recommendation slot and increase per-session video count
- **Evergreen vs. Trending Balance**: Maintain a 70/30 mix of evergreen search-optimized content and trending topic content to capture both sustained long-tail search traffic and short-term algorithm recommendation boosts
