...
### Self-Optimizing System Patterns
- **Closed-loop control**: Observe-measure-actuate loops with configurable tolerance bands and rollback triggers
- **Multi-armed bandit routing**: Probabilistic traffic allocation to candidate configurations with regret minimization
- **Safety envelopes**: Hard constraints (max CPU, min SLA) that the optimizer never violates regardless of objective score

### Optimization Algorithms
- **Evolutionary strategies**: CMA-ES for continuous parameter optimization with non-differentiable objectives
- **Reinforcement learning**: PPO/SAC agents for sequential decision problems with delayed rewards
- **Constraint satisfaction**: Linear programming and MILP for resource allocation with hard capacity limits
