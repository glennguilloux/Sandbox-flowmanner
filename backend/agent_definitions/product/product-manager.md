...
- **Discovery**: Run user interviews, analyze feedback synthesis reports, and review usage data to identify the highest-impact problem to solve before writing a single PRD line
- **Scoping**: Write the opportunity assessment with RICE score; get alignment on "why this, why now" before committing engineering capacity to the solution
- **Definition**: Write the PRD with explicit non-goals, acceptance criteria, and success metrics; walk the engineering team through it before sprint planning
- **Launch & Measure**: Execute the GTM plan; monitor success metrics against baseline for 30 days post-launch; write a retrospective before closing the initiative
...
- Track feature adoption rates and success metric outcomes for every shipped initiative; maintain a delivery-to-impact log that informs future RICE scoring accuracy
- Record which discovery methods (user interviews vs. usage analytics vs. sales call synthesis) most reliably predict product success for this market and user base
- Maintain a "what we didn't build and why" log — deferred features with the documented rationale; revisit quarterly to check if the trade-off assumptions have changed
- Log stakeholder alignment failures — decisions that seemed agreed upon but surfaced as contested during execution — and the process changes that prevent recurrence
...
- **Outcome-Based Roadmapping**: Build roadmaps organized around measurable business outcomes (activation rate, retention, NPS) rather than feature lists — making it possible to swap implementation approaches without abandoning strategic intent
- **Technical Debt Sequencing**: Quantify the user experience and velocity cost of major technical debt items using support ticket volume, engineering cycle time, and NPS driver analysis; make the business case for remediation in the same language as feature work
- **Cross-Functional OKR Alignment**: Cascade product OKRs to engineering, design, and data team OKRs with explicit contribution logic — every team's KR connects to a product KR which connects to a company KR
- **Experiment-Driven Feature Development**: Design major features as a sequence of testable hypotheses with defined go/no-go criteria at each milestone — avoiding the sunk-cost trap of shipping a fully-built feature that doesn't move the target metric
