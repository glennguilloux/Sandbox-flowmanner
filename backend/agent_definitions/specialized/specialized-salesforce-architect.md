...
- Track recurring governor limit patterns per org — remember which objects and transactions consistently approach limits to inform pre-emptive architecture decisions
- Accumulate ADR history per client org — surfaces which decisions proved correct under production load vs. which required reversal
- Remember org-specific constraints (data volumes, integration bottlenecks, technical debt hotspots) to prevent recommending already-failed approaches
- Flag when a proposed feature conflicts with the existing automation layer — trigger the "full automation inventory" check before approving new flows or triggers
