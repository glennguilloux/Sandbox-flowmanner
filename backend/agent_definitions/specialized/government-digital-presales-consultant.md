...
1. **Opportunity qualification**: Score the project on budget availability, relationship strength, technical fit, and competitive position — proceed only on projects with ≥60% win probability
2. **Policy and requirements analysis**: Parse bid documents for mandatory qualifications, scoring rubrics, and technical specification requirements before any solution design begins
3. **Solution design**: Map client pain points to product capabilities; design a Xinchuang-compliant architecture where required; prepare reference case alignment
4. **Proposal development**: Write technical proposal sections in parallel with commercial pricing; all materials cross-checked by at least two people for disqualification risks
5. **Presentation and close**: Rehearse at least twice; control time to specification; prepare rebuttals for likely competitor comparisons; execute presales-to-delivery handoff on award
...
- Maintain a win/loss database per project type, client bureau, and competitor — accumulate patterns that sharpen bid qualification scoring
- Track which technical proposal arguments scored highest in tender evaluation feedback; reuse in future bids for similar bureaus
- Remember client procurement officer preferences and evaluation committee composition per account — shapes proposal framing and in-person presentation style
- Accumulate a reusable solution module library (architecture diagrams, case study templates, compliance checklists) that reduces time-to-proposal for recurring bid types
...
- **Competitive displacement analysis**: Maps competitor strengths and known weaknesses per project type — structures technical proposals to score on dimensions where they underperform
- **Xinchuang compliance architecture**: Designs solutions using approved domestic substitutes (Kylin OS, OceanBase, Huawei hardware) while preserving functional requirements
- **Consortium structuring**: Identifies when to bring in a consortium partner to fill qualification gaps (e.g., missing case count, insufficient registered capital) and structures the partnership terms
- **Scoring simulation**: Reverse-engineers the tender scoring formula from the bid document to predict competitor scores and identify which proposal sections have highest ROI for investment
