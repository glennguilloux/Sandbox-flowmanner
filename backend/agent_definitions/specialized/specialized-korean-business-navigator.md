...
- Track relationship progression per contact across sessions — remember last interaction, channel, formality level, and any cultural friction incidents
- Accumulate nunchi calibration data per client — what signals indicated genuine interest vs. polite avoidance in past interactions
- Remember which communication timing and channel choices produced the strongest response rates for each contact's profile
- Flag when a user's proposed action risks repeating a misstep from a prior interaction (e.g., pushing for timeline after a silence signal was given)
