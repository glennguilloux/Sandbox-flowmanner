...
class TestMemoryCorrectionEventMetadata:
    ...
    def test_memory_correction_event_table_registered_in_metadata(self) -> None:
        ...
        from app.models.memory_correction_models import (  # noqa: F401
...
class TestMemoryCorrectionEventColumns:
    ...
    def test_required_columns_are_not_null(self) -> None:
        ...
        for col_name in required:
            assert cols[col_name].nullable is False, (
                f"MemoryCorrectionEvent.{col_name} must be NOT NULL "
                "(required field)"
            )
    ...
    def test_optional_columns_are_nullable(self) -> None:
        ...
        for col_name in optional:
            assert cols[col_name].nullable is True, (
                f"MemoryCorrectionEvent.{col_name} must be nullable "
                "(optional field)"
            )
    ...
    def test_id_is_uuid_primary_key(self) -> None:
        ...
        assert id_col.default is not None, (
            "MemoryCorrectionEvent.id must have a default (uuid4)"
        )
        assert id_col.default.is_callable is True, (
            "MemoryCorrectionEvent.id default must be callable"
        )
    ...
    def test_event_type_column_is_string(self) -> None:
        ...
        col = Base.metadata.tables[
            "memory_correction_events"
        ].columns["event_type"]
        assert col.type.__class__.__name__ == "String", (
            f"event_type must be String; got {col.type.__class__.__name__}"
        )
    ...
    def test_actor_column_is_string(self) -> None:
        ...
        assert col.type.__class__.__name__ == "String", (
            f"actor must be String; got {col.type.__class__.__name__}"
        )
    ...
    def test_details_column_is_jsonb(self) -> None:
        ...
        assert col.type.__class__.__name__ == "JSONB", (
            f"details must be JSONB; got {col.type.__class__.__name__}"
        )
...
class TestMemoryCorrectionEventIndexes:
    ...
    def test_index_on_user_workspace_event_type(self) -> None:
        ...
        assert any(
            cols == ("user_id", "workspace_id", "event_type")
            for _name, cols in index_column_sets
        ), (
            "Missing composite index on "
...
class TestMemoryCorrectionEventValueSets:
    ...
    def test_all_event_types_is_hardcoded_tuple(self) -> None:
        ...
        assert isinstance(ALL_EVENT_TYPES, tuple), (
            f"ALL_EVENT_TYPES must be a tuple; "
            f"got {type(ALL_EVENT_TYPES).__name__}"
        )
    ...
    def test_all_event_types_no_sunder_leak(self) -> None:
        ...
        for v in ALL_EVENT_TYPES:
            assert isinstance(v, str), (
                f"ALL_EVENT_TYPES entries must be str; "
                f"got {type(v).__name__}: {v!r}"
            )
            assert not v.startswith("_"), (
                f"ALL_EVENT_TYPES contains a sunder-name leak: {v!r}"
            )

    def test_all_actors_is_hardcoded_tuple(self) -> None:
        from app.models.memory_correction_models import ALL_ACTORS

        assert isinstance(ALL_ACTORS, tuple), (
            f"ALL_ACTORS must be a tuple; "
            f"got {type(ALL_ACTORS).__name__}"
        )
    ...
    def test_all_actors_no_sunder_leak(self) -> None:
        ...
        for v in ALL_ACTORS:
            assert isinstance(v, str), (
                f"ALL_ACTORS entries must be str; "
                f"got {type(v).__name__}: {v!r}"
            )
            assert not v.startswith("_"), (
                f"ALL_ACTORS contains a sunder-name leak: {v!r}"
            )
...
def test_default_actor_is_user() -> None:
    ...
    assert col.default is not None, (
        "actor must have a column-level default (=\"user\")"
    )
    # String literal default — SQLAlchemy represents it as a str.
    assert col.default.arg == "user", (
        f"actor default must be 'user'; got {col.default.arg!r}"
    )
...
def test_user_id_fk_to_users() -> None:
    ...
    user_col = Base.metadata.tables[
        "memory_correction_events"
    ].columns["user_id"]
    fk_targets = list(user_col.foreign_keys)
    assert len(fk_targets) == 1, (
        f"user_id must have exactly one FK; got {len(fk_targets)}"
    )
    fk = fk_targets[0]
...
def test_workspace_id_fk_with_cascade() -> None:
    ...
    workspace_col = Base.metadata.tables[
        "memory_correction_events"
...
def test_claim_id_fk_to_claims_with_set_null() -> None:
    ...
    claim_col = Base.metadata.tables[
        "memory_correction_events"
    ].columns["claim_id"]
    fk_targets = list(claim_col.foreign_keys)
    assert len(fk_targets) == 1, (
        f"claim_id must have exactly one FK; got {len(fk_targets)}"
    )
    fk = fk_targets[0]
    assert fk.column.table.name == "personal_memory_claims", (
        f"FK must target personal_memory_claims; got {fk.column.table.name}"
    )
    assert fk.ondelete == "SET NULL", (
        f"FK must ON DELETE SET NULL (audit row survives claim delete); "
        f"got {fk.ondelete!r}"
    )
