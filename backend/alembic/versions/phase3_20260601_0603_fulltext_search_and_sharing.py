...
def upgrade() -> None:
    ...
    op.execute("UPDATE chat_messages SET search_vector = to_tsvector('english', COALESCE(content, ''))")
    ...
    op.add_column("chat_threads", sa.Column("shared_with", postgresql.JSONB(), nullable=True))
