...
def downgrade() -> None:
    ...
    op.alter_column('roadmap_comments', 'user_id', existing_type=sa.String(36), type_=sa.Integer(), existing_nullable=False)
    op.alter_column('roadmap_votes', 'user_id', existing_type=sa.String(36), type_=sa.Integer(), existing_nullable=False)

    # Drop tables in reverse dependency order
    for table in ['runs', 'projects', 'workflow_runs', 'flows', 'tool_analytics',
                  'tool_permissions', 'custom_tools', 'tool_chain_executions', 'tool_chains',
                  'agent_registrations', 'webhook_logs', 'webhook_endpoints',
                  'learning_feedback', 'adaptation_rules']:
        op.drop_table(table)
