...
7. **Read routing is disabled (USE_NEW_READS pinned off).** `use_new_reads()` returns `False` unconditionally (kill switch, 2026-07-16), so query handlers always read from the legacy `Mission` table. The compat.py functions exist but are never reached via the flag. The Blueprint/Run model is dormant and unpopulated (write path removed 2026-07-07, `docs/DUAL-WRITE-DECISION.md`). Do not re-enable without a population/backfill decision.
