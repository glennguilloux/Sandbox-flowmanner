        ...
        templates.append(
            {
                ...
                "author_name": r[4],
                "category": r[5],
                "tags": _json(r[6]) if r[6] else [],
                "content": _json(r[7]) if r[7] else {},
                "rating": float(r[8] or 0),
                "rating_count": r[9] or 0,
                "fork_count": r[10] or 0,
                "use_count": r[11] or 0,
                "is_featured": r[12] or False,
                "created_at": _dt(r[13]) or "",
                "updated_at": _dt(r[14]) or "",
            }
        )
    return {
        "templates": templates,
...
async def get_template(
    ...
    r = await db.execute(
        text(
            "SELECT id, title, description, author_id, author_name, category, tags, content, rating, rating_count, fork_count, use_count, is_featured, created_at, updated_at FROM community_templates WHERE id=:id"
        ),
        {"id": template_id},
    )
    row = r.fetchone()
    ...
    return {
        "template": {
            ...
            "author_name": row[4],
            "category": row[5],
            "tags": _json(row[6]) if row[6] else [],
            "content": _json(row[7]) if row[7] else {},
            "rating": float(row[8] or 0),
            "rating_count": row[9] or 0,
            "fork_count": row[10] or 0,
            "use_count": row[11] or 0,
            "is_featured": row[12] or False,
            "created_at": _dt(row[13]) or "",
            "updated_at": _dt(row[14]) or "",
        }
    }
...
async def create_template(
    ...
    await db.execute(
        text(
            "INSERT INTO community_templates (id, title, description, author_id, author_name, category, tags, content, created_at, updated_at) VALUES (:id, :title, :desc, :aid, :aname, :cat, :tags, :content, NOW(), NOW())"
        ),
        {
            "id": tid,
            ...
            "aid": str(user.id),
            "aname": user.username if hasattr(user, "username") else str(user.id),
...
async def update_template(
    ...
    existing = await db.execute(
        text("SELECT author_id FROM community_templates WHERE id = :id"),
        {"id": template_id},
    )
    ...
    await db.execute(
        text("UPDATE community_templates SET " + ", ".join(sets) + " WHERE id = :id"),
        params,
    )
    await db.commit()
...
async def rate_template(
    ...
    await db.execute(
        text(
            "UPDATE community_templates SET rating = ((rating * rating_count) + :r) / (rating_count + 1), rating_count = rating_count + 1, updated_at=NOW() WHERE id=:id"
        ),
        {"id": template_id, "r": rating},
    )
    await db.commit()
    return await get_template(template_id, db, user)
...
async def fork_template(
    ...
    await db.execute(
        text("UPDATE community_templates SET fork_count = fork_count + 1 WHERE id=:id"),
        {"id": template_id},
    )
    await db.commit()
    return await get_template(template_id, db, user)
...
async def get_comments(
    ...
    count_r = await db.execute(
        text(
            f"SELECT COUNT(*) FROM community_comments c WHERE c.template_id = :tid AND c.parent_id IS NULL {deleted_filter}"
        ),
        {"tid": template_id},
    )
    total = count_r.scalar() or 0
...
async def add_comment(
    ...
    count_r = await db.execute(
        text(
            "SELECT COUNT(*) FROM community_comments WHERE template_id = :tid AND parent_id IS NULL AND is_deleted = false"
        ),
        {"tid": template_id},
    )
    total = count_r.scalar() or 0
...
async def get_featured(
    ...
    r = await db.execute(
        text(
            "SELECT id, title, description, author_id, author_name, category, tags, content, rating, rating_count, fork_count, use_count, is_featured, created_at, updated_at FROM community_templates WHERE is_featured=true ORDER BY rating DESC LIMIT 10"
        )
    )
    templates = []
    ...
    cats = [
        {
            ...
            "template_count": counts.get("research", 0),
            "slug": "research",
        },
        {
            "id": "support",
            ...
            "template_count": counts.get("support", 0),
            "slug": "support",
        },
        {
            "id": "sales",
            "name": "Sales",
            "description": "Sales and lead qualification templates",
            "template_count": counts.get("sales", 0),
            "slug": "sales",
        },
        {
            "id": "engineering",
            "name": "Engineering",
            "description": "Code review and dev templates",
            "template_count": counts.get("engineering", 0),
            "slug": "engineering",
        },
        {
            "id": "productivity",
            "name": "Productivity",
            "description": "Productivity and automation templates",
            "template_count": counts.get("productivity", 0),
            "slug": "productivity",
        },
    ]
...
async def get_trending(
    ...
    r = await db.execute(
        text(
            "SELECT id, title, description, author_id, author_name, category, tags, content, rating, rating_count, fork_count, use_count, is_featured, created_at, updated_at FROM community_templates ORDER BY use_count DESC LIMIT :lim"
        ),
        {"lim": limit},
    )
    templates = []
...
async def get_stats(
    ...
    r = await db.execute(
        text(
            "SELECT COUNT(*), COALESCE(SUM(use_count),0), COALESCE(SUM(fork_count),0), COALESCE(AVG(rating),0) FROM community_templates"
        )
    )
    row = r.fetchone()
