"""Custom Jira OAuth callback — handles Atlassian 3LO site discovery step.

Standard OAuth callbacks only exchange code for tokens. Jira Cloud requires
an additional site discovery step where we call
GET https://api.atlassian.com/oauth/token/accessible-resources
to find the user's Jira sites and extract the cloudId.
...
async def jira_oauth_callback(
    ...
    1. Discovers the user's Jira sites via accessible-resources
    ...
    if not provider:
        raise HTTPException(status_code=404, detail="Jira OAuth provider not configured")
    ...
    if resp.status_code != 200:
        raise HTTPException(
            status_code=502,
            detail=f"Jira token exchange failed: {resp.status_code} {resp.text[:200]}",
        )
    ...
    if not access_token:
        raise HTTPException(
            status_code=502,
            detail=f"Jira token exchange returned no access_token: {resp.text[:200]}",
        )

    # 3. Site discovery — find the user's Jira sites
    async with httpx.AsyncClient() as client:
    ...
    if sites_resp.status_code != 200:
        raise HTTPException(
            status_code=502,
            detail=f"Jira site discovery failed: {sites_resp.status_code} {sites_resp.text[:200]}",
        )
    ...
    if not sites:
        raise HTTPException(
            status_code=400,
            detail="No Jira sites found for this Atlassian account",
        )
    ...
    logger.info(
        "Jira OAuth callback: user %s connected to site '%s' (cloudId: %s, %d sites available)",
        stored["user_id"],
        account_name,
        cloud_id,
        len(sites),
    ...
    try:
        ...
        logger.warning("Failed to register Nexus capabilities for jira: %s", e)

    # 8. Redirect to frontend
    return RedirectResponse(
        url=f"https://flowmanner.com/dashboard/settings/integrations?connected=jira",
        status_code=302,
    )
