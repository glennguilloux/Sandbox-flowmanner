...
from ..openwhisk.integration_controller import OpenWhiskIntegrationController, create_integration_controller
...
class HealthCheck:
    ...
    async def comprehensive_health_check(self) -> dict[str, Any]:
        ...
        all_healthy = (
            openwhisk_health.get('status') in ('healthy', 'ok') and
            gateway_health.get('status') in ('healthy', 'ok') and
            actions_health.get('health_percentage', 0) >= 80
        )
