...
class OpenWhiskAuthManager:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        if not api_key:
            raise ValueError(
                "OpenWhisk API key is required. Set OPENWHISK_API_KEY "
...
def get_auth_manager() -> OpenWhiskAuthManager | None:
    ...
    try:
        ...
        logger.info(f"Auth manager ready (namespace: {manager.namespace})")
        return manager
    except ValueError as e:
        logger.warning(f"Auth manager not configured: {e}")
        return None
