...
class CatalogConfig:
    ...
    def validate(cls):
        ...
        logger.info(f"Catalog config validated - API URL: {cls.BACKEND_API_URL}")


try:
    CatalogConfig.validate()
except Exception as e:
    logger.warning(f"Catalog configuration issue: {e}")
...
class HTTPClient:
    ...
    def __init__(self, base_url: str, timeout: int = 10, max_retries: int = 2):
        ...
        logger.info(f"HTTP client initialized for catalog")
...
class CatalogClient:
    ...
    def __init__(self):
        ...
        logger.info(f"CatalogClient initialized for {self.config.BACKEND_API_URL}")
    ...
    def search_workflows(
        ...
        logger.info(
            f"Searching workflows - query: {query}, category: {category}, limit: {limit}"
        )
        ...
        try:
            ...
            logger.info(f"API search returned {len(workflows)} workflows")
            return workflows

        except CatalogConnectionError as e:
            logger.warning(f"API search failed, using fallback: {e}")
    ...
    def recommend_workflows(self, intent: str, limit: int = 5) -> list[dict]:
        ...
        logger.info(f"Recommending workflows - intent: {intent}, limit: {limit}")
        ...
        try:
            ...
            logger.info(
                f"API recommendations returned {len(recommendations)} workflows"
            )
            return recommendations

        except CatalogConnectionError as e:
            logger.warning(f"API recommendations failed, using fallback: {e}")
    ...
    def describe_workflow(self, workflow_id: str) -> dict:
        ...
        logger.info(f"Describing workflow - id: {workflow_id}")
        ...
        try:
            ...
            logger.warning(f"API describe failed, using fallback: {e}")
            return self._local_describe(workflow_id)
        except Exception as e:
            logger.error(f"Unexpected error in describe: {e}")
            return self._local_describe(workflow_id)
    ...
    def list_categories(self) -> list[dict]:
        ...
        try:
            ...
            logger.info(f"API returned {len(categories)} categories")
            return categories

        except CatalogConnectionError as e:
            logger.warning(f"API categories failed, using fallback: {e}")
            return self._local_categories()
        except Exception as e:
            logger.error(f"Unexpected error in categories: {e}")
            return self._local_categories()
    ...
    def _local_search(
        ...
        logger.info(f"Local search returned {len(results[:limit])} workflows")
    ...
    def _intent_recommend(self, intent: str, limit: int = 5) -> list[dict]:
        ...
        logger.info(f"Intent-based recommendations: {len(results[:limit])} workflows")
        return results[:limit]
    ...
    def _local_describe(self, workflow_id: str) -> dict:
        ...
        for wf in workflows:
            if wf.get("id") == workflow_id:
                logger.info(f"Found workflow in local catalog: {workflow_id}")
                return wf
        logger.warning(f"Workflow not found in local catalog: {workflow_id}")
    ...
    def _local_categories(self) -> list[dict]:
        ...
        logger.info(f"Local categories: {len(result)}")
        return result
...
def workflow_catalog(
    ...
    try:
        ...
        logger.error(f"workflow_catalog failed: {e}")
