...
class WebhookRouter:
    ...
    def register_default(self, name: str, handler: Callable, source: str) -> None:
        ...
        logger.info(f"Registered default webhook handler '{name}' for source '{source}'")
    ...
    async def route(self, source: str, event_type: str | None, payload: dict[str, Any], headers: dict[str, str]) -> dict[str, Any]:
        """Route a webhook to appropriate handlers"""
        handlers = self.get_handlers(source, event_type)
        
        if not handlers:
            logger.warning(f"No handlers found for source '{source}', event '{event_type}'")
            return {
                "success": False,
        ...
        for handler_info in handlers:
            try:
                logger.debug(f"Executing handler '{handler_info.name}' for {source}/{event_type}")
    ...
    def load_handler_module(self, module_path: str, function_name: str) -> Callable | None:
        ...
        try:
            ...
            logger.error(f"Handler function '{function_name}' not found in '{module_path}': {e}")
            return None
