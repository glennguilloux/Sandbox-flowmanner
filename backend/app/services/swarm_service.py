...
async def populate_swarm_from_division(db: AsyncSession, swarm_id: str, division: str) -> list[SwarmAgent]:
    ...
    logger.info(f"Populated swarm {swarm_id} with {len(agents)} agents from division '{division}'")
    return agents
...
async def populate_swarm_from_slugs(db: AsyncSession, swarm_id: str, slugs: list[str]) -> list[SwarmAgent]:
    ...
    for slug in slugs:
        ...
        if agent:
            agents.append(agent)
    logger.info(f"Populated swarm {swarm_id} with {len(agents)} agents from {len(slugs)} slugs")
    return agents
