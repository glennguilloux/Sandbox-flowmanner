...
class A2ASessionManager:
    ...
    async def create_session(
        ...
        async with self._lock:
            ...
            logger.info(f"Created A2A session {session.id} for agent {agent_url}")
            return session
    ...
    async def close_session(self, session_id: str) -> bool:
        ...
        async with self._lock:
            if session_id in self._sessions:
                ...
                logger.info(f"Closed A2A session {session_id}")
                return True
            return False
...
class A2AServer:
    ...
    async def _handle_task(self, message: A2AMessage, session_id: str) -> A2AMessage:
        """Handle task message"""
        logger.info(f"Task received: {message.id} from {message.sender}")
    ...
    async def _handle_file(self, message: A2AMessage, session_id: str) -> A2AMessage:
        ...
        logger.info(f"File received: {message.attachments}")
        return message
