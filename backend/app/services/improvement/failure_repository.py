...
class FailureRepository:
    ...
    async def save(self, context: FailureContext) -> str:
        ...
        logger.debug(f"Saved failure {context.failure_id} of type {context.failure_type.value}")
    ...
    async def clear_old_failures(self, older_than: timedelta = timedelta(days=30)) -> int:
        ...
        # Clear from memory
        to_remove = [
            fid for fid, context in self._memory_store.items()
    ...
    async def _get_from_database(self, failure_id: str) -> FailureContext | None:
        ...
        model = self.db.query(FailureContextModel).filter(
            FailureContextModel.failure_id == failure_id
        ).first()
