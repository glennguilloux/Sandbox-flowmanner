...
class KnowledgeGraph:
    ...
    async def add_edge(
        ...
        logger.debug(f"Added edge {edge_id} ({source_id} -{edge_type.value}-> {target_id})")
        return edge
    ...
    async def query(
        ...
        if start_key:
            ...
            start_nodes = [
                self._nodes[nid] 
                for nid in self._node_index[start_type].values()
            ]
    ...
    async def load_from_database(self) -> None:
        ...
        try:
            ...
            node_result = await self.db_session.execute(
                text("SELECT id, node_type, node_key, properties, created_at, updated_at FROM improvement_knowledge_nodes")
            )
            ...
            edge_result = await self.db_session.execute(
                text("SELECT id, source_id, target_id, edge_type, weight, properties, created_at FROM improvement_knowledge_edges")
            )
