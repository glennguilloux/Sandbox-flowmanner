...
class ImprovementKnowledge:
    ...
    def record_success(self, strategy: ImprovementStrategy, improvement_delta: float):
        ...
        if strategy.strategy_type not in self.strategy_effectiveness:
            self.strategy_effectiveness[strategy.strategy_type] = {"success": 0, "failure": 0}
...
class ImprovementLoopV2:
    ...
    async def on_mission_complete(
        ...
        """
        logger.info(f"Mission complete hook: mission={mission_id}, success={success}")
    ...
    async def on_failure(
        ...
        logger.debug(f"Failure buffered: {failure_context.failure_type.value}, buffer size: {len(self._failure_buffer)}")
    ...
    async def run_improvement_session(
        ...
        try:
            ...
            if session.test_result.recommendation == "apply":
                ...
                if session.test_result.state == HypothesisState.ROLLED_BACK:
                    ...
                    session.notes = f"Improvement not applied: {session.test_result.recommendation}"
                
                self.knowledge.record_failure(session.selected_strategy)
                session.state = SessionState.COMPLETED
            
        except Exception as e:
            logger.error(f"Improvement session failed: {e}")
            session.state = SessionState.FAILED
            session.notes = str(e)
        ...
        logger.info(f"Improvement session {session.session_id} completed: {session.state.value}")
        return session
        ...
        if len(failures) < self.min_failures_for_analysis and self.db_session:
            try:
                db_contexts = await self.causal_decomposer.decompose_failures(
                    WeakArea(area_type="auto", success_rate=0.0, total_attempts=0, failure_count=0),
    ...
    async def _get_current_metrics(
        ...
        try:
            ...
            base_query = select(
                ...
                func.sum(
                    case((Mission.status == "completed", 1), else_=0)
                ).label("completed"),
                func.sum(
                    case((Mission.status == "failed", 1), else_=0)
                ).label("failed"),
                func.avg(
                    func.extract(
                        "epoch", Mission.completed_at - Mission.started_at
                    )
                ).label("avg_duration"),
            )
            ...
            if agent_id:
                try:
                    ...
                    logger.warning("agent_id %s is not a valid integer user ID, querying all missions", agent_id)
            ...
            try:
                ...
                tool_query = select(
                    ...
                    func.sum(
                        case((ToolAnalytics.success == True, 1), else_=0)
                    ).label("successful_tool_calls"),
                )
                ...
                logger.warning("Failed to query tool analytics: %s, using default tool_success_rate", tool_e)
    ...
    def get_knowledge_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "knob_effectiveness": {
                kt.value: data
                for kt, data in self.knowledge.knob_effectiveness.items()
            },
