...
class WeakArea:
    ...
    time_window: tuple[datetime, datetime] = field(default_factory=lambda: (datetime.now(UTC) - timedelta(days=7), datetime.now(UTC)))
...
STRATEGY_MAP: dict[FailureType, list[ImprovementStrategy]] = {
    ...
    FailureType.TOOL_API_ERROR: [
        ImprovementStrategy(
            ...
            knob_value={"max_retries": 3, "backoff_factor": 2, "retry_on": ["5xx", "timeout", "connection_error"]},
        ...
        ImprovementStrategy(
            ...
            applicable_failure_types=[FailureType.TOOL_API_ERROR, FailureType.TOOL_TIMEOUT],
        ),
        ImprovementStrategy(
            strategy_type=StrategyType.ADD_CIRCUIT_BREAKER,
            knob=KnobType.AGENT_BEHAVIOR,
            knob_value={"circuit_breaker_enabled": True, "failure_threshold": 5, "recovery_timeout": 60},
            description="Add circuit breaker to prevent cascading failures",
            estimated_impact=0.20,
            risk_level=RiskLevel.MEDIUM,
            confidence=0.75,
            rollback_value={"circuit_breaker_enabled": False},
            applicable_failure_types=[FailureType.TOOL_API_ERROR, FailureType.CONNECTION_FAILURE],
        ),
    ],
    ...
    FailureType.RESOURCE_EXHAUSTION: [
        ImprovementStrategy(
            ...
            applicable_failure_types=[FailureType.RESOURCE_EXHAUSTION, FailureType.RATE_LIMITED],
        ),
    ],
    ...
    FailureType.TOOL_INVALID_INPUT: [
        ImprovementStrategy(
            ...
            knob_value={"validation_enabled": True, "strict_mode": False, "coerce_types": True},
    ...
    FailureType.TOOL_INVALID_OUTPUT: [
        ImprovementStrategy(
            ...
            knob=KnobType.TOOL_SCHEMA,
            knob_value={"output_validation_enabled": True, "required_fields": [], "fail_on_invalid": False},
        ...
        ImprovementStrategy(
            ...
            knob=KnobType.AGENT_BEHAVIOR,
            knob_value={"graceful_degradation": True, "default_on_failure": {"success": False, "error": "Tool output invalid"}},
    ...
    FailureType.LLM_REFUSAL: [
        ...
        ImprovementStrategy(
            strategy_type=StrategyType.SWITCH_TO_CAPABLE_MODEL,
            knob=KnobType.MODEL_ROUTING,
            knob_value={"model": "claude-3-opus", "reason": "better_instruction_following"},
    ...
    FailureType.WORKFLOW_DEPENDENCY_FAIL: [
        ImprovementStrategy(
            strategy_type=StrategyType.ADD_FALLBACK_HANDLER,
            knob=KnobType.AGENT_BEHAVIOR,
            knob_value={"dependency_fallback": True, "skip_on_failure": True, "notify_on_skip": True},
...
class CausalDecomposer:
            ...
            for imp in improvements:
                if imp.failure_type and imp.failure_context:
                    ...
                    ctx = FailureContext(
                        ...
                        context_data={"improvement_id": imp.id, "detail": imp.failure_context},
                    )
                    contexts.append(ctx)
    ...
    def select_strategies(
        ...
        logger.info(
            f"Selected {len(selected_strategies)} strategies from {len(failures)} failures "
            f"across {len(failure_counts)} failure types"
        )
