...
class HypothesisTest:
    ...
    rollback_triggers: list[RollbackTrigger] = field(default_factory=lambda: [
...
class HypothesisTester:
    ...
    async def create_test(
        ...
        logger.info(f"Created hypothesis test {test.test_id} for strategy {strategy.strategy_id}")
    ...
    async def run_pre_deployment_check(
        ...
        knob = await self.knob_manager.get_knob(strategy.knob, strategy.applicable_failure_types[0].value if strategy.applicable_failure_types else None)
    ...
    async def start_test(
        ...
        if test.state != HypothesisState.PENDING:
            logger.warning(f"Test {test.test_id} is not in PENDING state")
            return False
        ...
        if test.strategy:
            ...
            if not can_deploy:
                logger.error(f"Pre-deployment check failed for test {test.test_id}: {issues}")
        ...
        if test.strategy:
            ...
            if not adjustment:
                logger.error(f"Failed to apply strategy for test {test.test_id}")
        ...
        logger.info(f"Started hypothesis test {test.test_id}")
        return True
    ...
    async def rollback_test(
        ...
        if test.strategy:
            ...
            if success:
                ...
                logger.info(f"Rolled back test {test.test_id} due to {trigger.value}")
                return True
    ...
    async def _complete_test(
        ...
        logger.info(
            f"Completed test {test.test_id}: {test.state.value}, "
            f"improvement={test.improvement_delta:.2%}"
        )
