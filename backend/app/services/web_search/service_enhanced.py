    ...
    def _init_providers(self):
        ...
        if self.config.duckduckgo_enabled:
            try:
                ...
                logger.warning(f"Failed to init duckduckgo: {e}")
        ...
        if self.config.searxng_enabled and self.config.searxng_url:
            try:
                ...
                logger.warning(f"Failed to init searxng: {e}")
    ...
    async def search(
        ...
        if use_query_understanding:
            try:
                ...
                logger.info(f"Query intent: {query_understanding.intent.value}, "
                           f"complexity: {query_understanding.complexity_score:.2f}")
            except Exception as e:
        ...
        if query_understanding:
            if query_understanding.intent in (SearchIntent.RESEARCH, SearchIntent.COMPARISON):
