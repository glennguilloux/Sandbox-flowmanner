...
class ApprovalWorkflow:
    ...
    def _load_request_from_redis(self, request_id: str) -> ApprovalRequest | None:
        ...
        try:
            ...
            logger.debug(f"Loaded approval request {request_id} from Redis")
            return request
        except Exception as e:
            logger.warning(f"Failed to load approval request from Redis: {e}")
            return None
    ...
    def approve(
        ...
        logger.info(f"Approved tool execution {request_id} by user {user_id}")
    ...
    def reject(
        ...
        logger.info(f"Rejected tool execution {request_id} by user {user_id}: {reason}")
    ...
    def cancel(
        ...
        logger.info(f"Cancelled tool execution {request_id} by user {user_id}")
