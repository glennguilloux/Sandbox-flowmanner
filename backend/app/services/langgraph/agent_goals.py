...
class GoalExecutor:
    ...
    async def execute_step(self, goal: AgentGoal, step: GoalStep, user_context: dict[str, Any] = None) -> dict[str, Any]:
        """
        ...
        if result.get("success"):
            ...
            for i in range(step_index + 1, len(goal.steps)):
                ...
                if next_step.status == StepStatus.FAILED:
                    goal.status = GoalStatus.FAILED
                    return {"success": False, "error": next_step.error, "goal": goal.to_dict()}
