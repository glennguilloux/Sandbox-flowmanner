...
class LangGraphAgent:
    ...
    def _register_default_handlers(self, config: dict[str, Any]):
        ...
        if n8n_base_url:
            ...
            logger.info(f"Registered n8n handler with URL: {n8n_base_url}")
        ...
        if comfyui_base_url:
            ...
            logger.info(f"Registered ComfyUI handler with URL: {comfyui_base_url}")
    ...
    def register_tool_handler(
        ...
        logger.debug(f"Registered handler for tool: {tool_id}")
    ...
    async def _process_input_node(self, state: AgentState) -> AgentState:
        """Process user input"""
        logger.debug(f"Processing input for session {state['session_id']}")
        return state
    ...
    async def _check_approval_node(self, state: AgentState) -> AgentState:
        ...
        if state["pending_tools"]:
            ...
            if result["success"]:
                ...
                logger.info(f"Approval workflow created request for tool {tool['tool_name']}")
            else:
                logger.warning(f"Failed to create approval request: {result.get('error')}")
    ...
    def _handle_n8n_workflow(
        ...
        from app.services.langchain.tools.n8n_agent_tool_prod import execute_n8n_workflow
    ...
    def _handle_comfyui_workflow(
        ...
        from app.services.langchain.tools.comfyui_agent_tool_prod import generate_hero_background
    ...
    def _handle_3dglenn_workflow(
        ...
        from app.services.langchain.tools.comfyui_agent_tool_prod import generate_3d_model
        ...
        try:
            ...
            logger.error(f"Error executing 3Dglenn workflow: {e}")
            return {"success": False, "error": str(e)}
    ...
    if _agent is None:
        ...
        if llm is None:
            try:
                ...
                logger.error(f"Failed to auto-initialize LLM: {e}")
