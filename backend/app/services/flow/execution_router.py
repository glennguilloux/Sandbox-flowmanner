...
class ExecutionRouter:
    ...
    def __init__(self):
        ...
        self.mission_keywords = [
            "execute", "complete", "task", "mission", "step", "process",
            "accomplish", "achieve", "finish", "run task"
        ]
        self.workflow_keywords = [
            "workflow", "pipeline", "chain", "sequence", "automate",
            "orchestrate", "series of steps"
        ]
        self.ai_keywords = [
            "write", "generate", "create", "analyze", "summarize", "explain",
            "help", "chat", "answer", "research", "draft"
        ]
    ...
    async def route_and_execute(
        ...
        if explicit_route:
            logger.info(f"Using explicit route: {explicit_route} for project {project['slug']}")
            route = explicit_route
        else:
            # Analyze goal to determine route
            route = self.analyze_goal(goal)
            logger.info(f"Analyzed route: {route} for goal: {goal[:50]}...")
    ...
    async def _execute_mission(
        ...
        try:
            ...
            logger.error(f"Mission route error: {e.response.status_code} - {e.response.text}")
            raise Exception(f"Mission execution failed: {e.response.text}")
        except Exception as e:
            logger.error(f"Mission route exception: {e}")
            raise
    ...
    async def _execute_workflow(
        ...
        try:
            ...
            logger.error(f"Workflow route error: {e.response.status_code} - {e.response.text}")
            raise Exception(f"Workflow execution failed: {e.response.text}")
        except Exception as e:
            logger.error(f"Workflow route exception: {e}")
            raise
    ...
    async def _execute_ai(
        ...
        try:
            ...
            logger.error(f"AI route error: {e.response.status_code} - {e.response.text}")
            raise Exception(f"AI execution failed: {e.response.text}")
        except Exception as e:
            logger.error(f"AI route exception: {e}")
            raise
