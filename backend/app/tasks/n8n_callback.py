...
def wait_for_n8n_callback(self, execution_id: str, timeout: int = None) -> dict[str, Any]:
    ...
    logger.info(f"Waiting for n8n callback: execution_id={execution_id}, timeout={timeout}s")
    
    while True:
        elapsed = (datetime.now(UTC) - start_time).total_seconds()
        
        if elapsed >= timeout:
            logger.warning(f"N8N callback timeout: execution_id={execution_id}")
            return {
                "status": "timeout",
        ...
        if callback_data:
            try:
                ...
                logger.info(f"N8N callback received: execution_id={execution_id}, status={result.get('status')}")
                ...
                logger.error(f"Invalid callback data for {execution_id}: {e}")
                return {
                    "status": "error",
                    "execution_id": execution_id,
...
def store_n8n_callback(execution_id: str, callback_data: dict[str, Any]) -> bool:
    ...
    logger.info(f"Stored n8n callback: execution_id={execution_id}")
...
def cleanup_stale_callbacks() -> int:
    ...
    if cleaned > 0:
        logger.info(f"Cleaned up {cleaned} stale callback entries")
