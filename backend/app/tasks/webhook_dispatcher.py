...
from ..models.webhook_subscription import WebhookDelivery, WebhookEventType, WebhookSubscription
...
async def deliver_webhook_async(
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session, session.request(
                ...
                if 200 <= response.status < 300:
                    ...
                    logger.info(f"Webhook delivered successfully to {subscription.endpoint_url}")
                    return True
                else:
                    # HTTP error
                    ...
                    logger.warning(f"Webhook delivery failed with HTTP {response.status}")
                        
                
        except TimeoutError:
            delivery.status = "failed"
            delivery.error_message = f"Timeout after {subscription.timeout_seconds}s"
            logger.warning(f"Webhook delivery timed out")
            ...
            logger.error(f"Webhook delivery unexpected error: {e}")
    ...
    if subscription.consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
        ...
        logger.warning(f"Disabled webhook subscription {subscription.id} due to {MAX_CONSECUTIVE_FAILURES} consecutive failures")
...
def dispatch_webhook_event(event_type: str, event_id: str, payload: dict) -> dict:
    ...
    """
    logger.info(f"Dispatching webhook event: {event_type} (id={event_id})")
    ...
    try:
        ...
        logger.info(f"Webhook dispatch complete: {success_count} success, {failure_count} failed")
...
def retry_failed_webhooks() -> dict:
    ...
    try:
        ...
        for delivery in pending:
            ...
            # Re-dispatch
            result = asyncio.run(deliver_webhook_async(
                subscription,
                delivery.event_type,
                delivery.event_id,
                delivery.payload,
...
def emit_agent_approval_required(execution_id: int, user_id: int, tool_name: str, params: dict):
    emit_event(
        ...
        {"execution_id": execution_id, "user_id": user_id, "tool_name": tool_name, "params": params}
    )
