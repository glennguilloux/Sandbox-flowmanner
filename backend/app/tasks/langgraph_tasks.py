...
def langgraph_execute_task(self, message: str, session_id: str | None = None, 
    ...
    """
    logger.info(f"Starting LangGraph task for message: {message[:100]}...")
    ...
    try:
        ...
        logger.info(f"LangGraph task completed successfully: {self.request.id}")
        return result
        
    except Exception as e:
        logger.error(f"LangGraph task failed: {e}", exc_info=True)
...
def langgraph_approval_task(self, session_id: str, approval: bool, 
    ...
    """
    logger.info(f"Processing LangGraph approval for session: {session_id}")
    ...
    try:
        ...
        logger.info(f"LangGraph approval task completed: {self.request.id}")
        return result
        
    except Exception as e:
        logger.error(f"LangGraph approval task failed: {e}", exc_info=True)
...
def langgraph_tool_execution_task(self, tool_name: str, parameters: dict[str, Any],
                                 context: dict[str, Any] | None = None) -> dict[str, Any]:
    """
    ...
    """
    logger.info(f"Executing LangGraph tool: {tool_name}")
    ...
    try:
        ...
        logger.info(f"LangGraph tool execution completed: {tool_name}")
        return result
        
    except Exception as e:
        logger.error(f"LangGraph tool execution failed: {e}", exc_info=True)
...
def langgraph_batch_process_task(self, messages: list, session_id: str | None = None,
                                context: dict[str, Any] | None = None) -> dict[str, Any]:
    """
    ...
    """
    logger.info(f"Starting LangGraph batch processing for {len(messages)} messages")
    ...
    try:
        ...
        for i, message in enumerate(messages):
            ...
            try:
                ...
                logger.error(f"Failed to process batch message {i+1}: {e}")
                results.append({
                    "message": message,
                    "error": str(e),
                    "success": False
        ...
        logger.info(f"LangGraph batch processing completed: {len(results)} messages")
        return {
        ...
        logger.error(f"LangGraph batch processing failed: {e}", exc_info=True)
