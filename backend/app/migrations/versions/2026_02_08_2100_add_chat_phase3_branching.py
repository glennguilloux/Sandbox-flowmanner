...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('merge_status', sa.String(20), default='pending', index=True),  # pending, in_progress, completed, failed
        ...
        sa.ForeignKeyConstraint(['destination_branch_id'], ['chat_thread_branches.branch_id'], ondelete='CASCADE'),
    ...
    op.create_index('idx_branch_merges_source', 'chat_branch_merges', ['source_branch_id'], unique=False)
    op.create_index('idx_branch_merges_destination', 'chat_branch_merges', ['destination_branch_id'], unique=False)
    ...
    op.create_index('idx_branch_comparisons_comparison_id', 'chat_branch_comparisons', ['comparison_id'], unique=True)
    op.create_index('idx_branch_comparisons_branch_a', 'chat_branch_comparisons', ['branch_a_id'], unique=False)
    op.create_index('idx_branch_comparisons_branch_b', 'chat_branch_comparisons', ['branch_b_id'], unique=False)
    op.create_index('idx_branch_comparisons_type', 'chat_branch_comparisons', ['comparison_type'], unique=False)
    ...
    op.create_index('idx_branch_hierarchy_hierarchy_id', 'chat_branch_hierarchy', ['hierarchy_id'], unique=True)
    op.create_index('idx_branch_hierarchy_parent', 'chat_branch_hierarchy', ['parent_branch_id'], unique=False)
    op.create_index('idx_branch_hierarchy_child', 'chat_branch_hierarchy', ['child_branch_id'], unique=False)
    op.create_index('idx_branch_hierarchy_level', 'chat_branch_hierarchy', ['hierarchy_level'], unique=False)
    ...
    op.create_index('idx_branch_analytics_analytics_id', 'chat_branch_analytics', ['analytics_id'], unique=True)
    op.create_index('idx_branch_analytics_branch', 'chat_branch_analytics', ['branch_id'], unique=False)
    op.create_index('idx_branch_analytics_type', 'chat_branch_analytics', ['metric_type'], unique=False)
    op.create_index('idx_branch_analytics_recorded', 'chat_branch_analytics', ['recorded_at'], unique=False)
    
    # Add enhanced columns to chat_thread_branches
    op.add_column('chat_thread_branches', sa.Column('merge_status', sa.String(20), default='none', index=True))  # none, pending, merged
    op.add_column('chat_thread_branches', sa.Column('merge_destination_id', sa.String(64), nullable=True, index=True))
    op.add_column('chat_thread_branches', sa.Column('branch_order', sa.Integer(), default=0))
    op.add_column('chat_thread_branches', sa.Column('branch_color', sa.String(7), nullable=True))  # Hex color for visualization
    op.add_column('chat_thread_branches', sa.Column('message_count', sa.Integer(), default=0))
    op.add_column('chat_thread_branches', sa.Column('last_accessed', sa.DateTime(timezone=True), nullable=True))
    
    # Add index for merge_status
    op.create_index('idx_branches_merge_status', 'chat_thread_branches', ['merge_status'], unique=False)
    
    # Add branch_id column to chat_messages to track which branch a message belongs to
    op.add_column('chat_messages', sa.Column('branch_id', sa.String(64), nullable=True, index=True))
