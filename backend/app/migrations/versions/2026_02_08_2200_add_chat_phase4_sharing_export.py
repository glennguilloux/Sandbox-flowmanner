...
def upgrade():
    ...
    op.create_index('idx_share_analytics_analytics_id', 'share_analytics', ['analytics_id'], unique=True)
    ...
    op.create_index('idx_export_analytics_analytics_id', 'export_analytics', ['analytics_id'], unique=True)
    ...
    op.create_index('idx_export_analytics_recorded', 'export_analytics', ['recorded_at'], unique=False)
    ...
    op.add_column('chat_threads', sa.Column('default_share_token', sa.String(64), nullable=True, unique=True))
    op.add_column('chat_threads', sa.Column('default_share_permission', sa.String(20), default='view'))
    op.add_column('chat_threads', sa.Column('default_share_expires_at', sa.DateTime(timezone=True), nullable=True))
    
    # Create index for default_share_token
    op.create_index('idx_threads_default_share_token', 'chat_threads', ['default_share_token'], unique=True)
