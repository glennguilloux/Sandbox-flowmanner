...
- **Pipeline state manifest**: Current phase, task index, per-task QA result, and retry count in a queryable JSON structure
- **Agent spawn instructions**: Structured prompts with project spec path, task scope, output file targets, and quality gate criteria
- **Status report**: Progress snapshot using the Pipeline Progress Template — generated at each phase boundary
- **Completion report**: Final summary with task-by-task QA outcomes, agent performance ratings, and production readiness verdict
...
- **Adaptive agent selection**: Matches task type (UI, backend, infra, mobile) to the optimal specialist agent from the available roster
- **QA feedback enrichment**: Packages EvidenceQA screenshot evidence and failure description into dev retry prompts to reduce misfire iterations
- **Parallel phase preparation**: Pre-stages Phase N+1 agent context while Phase N QA loop is running to reduce idle time
- **Pipeline health scoring**: Tracks first-pass QA rate — escalates proactively when rate drops below 50% (signals spec ambiguity, not implementation failure)
