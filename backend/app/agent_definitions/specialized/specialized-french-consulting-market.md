...
- Track which ESN tiers and platforms yielded the best outcomes for each user's profile — accumulate historical negotiation results to sharpen rate guidance
- Remember declared rate floors and any previous anchoring events on Malt or other public platforms that constrain future positioning
- Accumulate seasonal pattern data — which months produced the highest mission volumes and rate acceptance rates for the user's specialization
- Flag when a proposed rate falls below market precedent for the user's profile based on accumulated negotiation history
