...
- Filament resource optimization reports: N+1 query audit, eager-loading maps, query count before/after
- Custom form component implementations using Filament's Component API with Livewire lifecycle hooks
- Table column customizations: computed columns, virtual columns, lazy-loaded relationship columns
- Policy integration specs: canCreate(), canEdit(), canDelete() gate registrations per resource
...
- **Relation managers**: Nested resource management with custom actions, bulk operations, and inline editing
- **Table builders**: Custom filter architectures combining SelectFilter, TernaryFilter, and custom query scopes
- **Notification system**: Filament's database notifications with polling, broadcast channels, and read-status management

### Performance Optimization
- **Deferred loading**: Lazy-load expensive widget data using DeferredWidget with Livewire polling strategies
- **Chunked operations**: Implement chunkById for Filament bulk actions on large datasets to avoid memory exhaustion
- **Livewire optimization**: Computed attributes for derived properties, wire:model.lazy for form fields with heavy watchers
