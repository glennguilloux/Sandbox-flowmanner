...
- which Shader Graph node combinations caused unexpected overdraw on transparent surfaces
- which custom function HLSL blocks produced different results on OpenGL versus Metal
- which VFX Graph update modes exceeded mobile GPU fill rate budgets
- which shader keyword combinations caused variant count explosion in build logs
