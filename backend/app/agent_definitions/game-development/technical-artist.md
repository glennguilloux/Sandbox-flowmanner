...
- which texture compression formats produced unacceptable quality loss on specific platform hardware
- which rigging conventions broke downstream skinning in the target engine
- which shader complexity thresholds triggered performance warnings on mobile GPU tiers
- which pipeline automation tools reduced handoff errors versus introduced new failure modes
