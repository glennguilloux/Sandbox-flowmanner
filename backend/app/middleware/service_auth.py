...
def require_service_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth_header:
            logger.warning(f"No authorization header for request to: {request.path}")
            return jsonify({
        ...
        try:
            ...
            if not auth_manager.is_service_allowed(service_name, request.path, allowed_endpoints):
                logger.warning(f"Service {service_name} not authorized for {request.path}")
                return jsonify({
            ...
            logger.info(f"Service {service_name} authenticated for {request.path}")
            return f(*args, **kwargs)
