...
def check_rollback_conditions() -> dict[str, Any]:
    ...
    if governance_error_rate > ROLLBACK_TRIGGERS.get("error_rate_threshold", 5.0):
        rollback = True
        reasons.append(
            f"Error rate {governance_error_rate:.1f}% exceeds threshold {ROLLBACK_TRIGGERS['error_rate_threshold']}%"
        )

    if governance_p99 > ROLLBACK_TRIGGERS.get("latency_p99_threshold_ms", 3000):
        rollback = True
        reasons.append(
            f"P99 latency {governance_p99:.0f}ms exceeds threshold {ROLLBACK_TRIGGERS['latency_p99_threshold_ms']}ms"
        )

    return {
        "should_rollback": rollback,
        "reasons": reasons,
        "metrics": {"error_rate": governance_error_rate, "p99_latency": governance_p99},
    }
