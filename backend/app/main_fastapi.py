...
# The catch-all Exception handler now lives in app/api/_shared_errors.py
# (register_unified_exception_handlers -> unified_exception_handler), which is
# the single source of truth for unhandled exceptions AND the only place that
# fires a scoped ntfy alert (AUDIT-B12). A prior general_error_handler here was
# shadowed by that unified handler (FastAPI keys handlers by exception class;
# last registration wins) and is intentionally removed to avoid the duplicate /
# unreachable alert path.
