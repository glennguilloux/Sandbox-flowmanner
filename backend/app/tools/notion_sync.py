...
class NotionSyncTool(BaseTool):
    ...
    async def execute(self, input_data: dict) -> ToolResult:
        ...
        if not token:
            return ToolResult.error_result(tool_id=self.tool_id, error="No Notion token. Set NOTION_TOKEN or pass token.")
    ...
    async def _check_error(self, r: httpx.Response) -> ToolResult | None:
        if r.status_code not in (200, 201):
            return ToolResult.error_result(tool_id=self.tool_id, error=f"Notion API error {r.status_code}: {r.text[:300]}")
        return None
    ...
    async def _query_database(self, client, base_url, v) -> ToolResult:
        ...
        if err := await self._check_error(r):
            return err
        data = r.json()
        pages = [{"id": p["id"], "properties": p.get("properties", {}),
                   "url": p.get("url"), "created_time": p.get("created_time")}
                  for p in data.get("results", [])]
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "query_database", "count": len(pages), "has_more": data.get("has_more"), "pages": pages,
        })
    ...
    async def _get_page(self, client, base_url, v) -> ToolResult:
        ...
        p = r.json()
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "get_page", "page": {"id": p["id"], "properties": p.get("properties", {}),
                                           "url": p.get("url"), "created_time": p.get("created_time")},
        })
    ...
    async def _create_page(self, client, base_url, v) -> ToolResult:
        ...
        payload: dict = {"parent": {"database_id": v.database_id}, "properties": v.properties}
        if v.children:
        ...
        p = r.json()
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "create_page", "page": {"id": p["id"], "url": p.get("url")},
        })
    ...
    async def _get_block_children(self, client, base_url, v) -> ToolResult:
        ...
        data = r.json()
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "get_block_children", "count": len(data.get("results", [])), "blocks": data.get("results"),
        })
    ...
    async def _search(self, client, base_url, v) -> ToolResult:
        ...
        if err := await self._check_error(r):
            return err
        data = r.json()
        results = [{"id": i["id"], "object": i.get("object"), "title": str(i.get("properties", {}).get("title", i.get("id"))),
                     "url": i.get("url")} for i in data.get("results", [])]
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "search", "query": v.query, "count": len(results), "results": results,
        })
