...
class GoogleWorkspaceHubTool(BaseTool):
        ...
        for msg in messages[:min(v.max_results, 10)]:
            ...
            if detail.status_code == 200:
                ...
                emails.append({
                    ...
                    "date": headers.get("Date", ""), "snippet": snippet,
                })
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "list_emails", "count": len(emails), "emails": emails,
        })
    ...
    async def _get_email(self, client: httpx.AsyncClient, v) -> ToolResult:
        ...
        r = await client.get(f"https://gmail.googleapis.com/gmail/v1/users/me/messages/{v.email_id}",
                             params={"format": "full"})
        if r.status_code != 200:
            return ToolResult.error_result(tool_id=self.tool_id, error=f"Gmail API error: {r.status_code}")
        d = r.json()
        ...
        return ToolResult.success_result(tool_id=self.tool_id, result={
            "action": "get_email", "id": d["id"], "subject": headers.get("Subject"),
    ...
    async def _send_email(self, client: httpx.AsyncClient, v) -> ToolResult:
        ...
        r = await client.post("https://gmail.googleapis.com/gmail/v1/users/me/messages/send",
                              json={"raw": raw})
        if r.status_code != 200:
            return ToolResult.error_result(tool_id=self.tool_id, error=f"Gmail API error: {r.status_code} {r.text[:200]}")
        return ToolResult.success_result(tool_id=self.tool_id, result={"action": "send_email", "ok": True, "id": r.json().get("id")})

    async def _list_events(self, client: httpx.AsyncClient, v) -> ToolResult:
        r = await client.get(f"https://www.googleapis.com/calendar/v3/calendars/{v.calendar_id}/events",
                             params={"maxResults": v.max_results, "orderBy": "startTime",
                                     "singleEvents": "true", "timeMin": v.event_start or "2024-01-01T00:00:00Z"})
        if r.status_code != 200:
            return ToolResult.error_result(tool_id=self.tool_id, error=f"Calendar API error: {r.status_code}")
        data = r.json()
        events = [{"id": e["id"], "summary": e.get("summary"), "start": e.get("start", {}).get("dateTime", e.get("start", {}).get("date")),
                    "end": e.get("end", {}).get("dateTime", e.get("end", {}).get("date")),
                    "location": e.get("location")} for e in data.get("items", [])]
        return ToolResult.success_result(tool_id=self.tool_id, result={"action": "list_events", "count": len(events), "events": events})
    ...
    async def _create_event(self, client: httpx.AsyncClient, v) -> ToolResult:
        if not v.event_summary or not v.event_start or not v.event_end:
            return ToolResult.error_result(tool_id=self.tool_id, error="event_summary, event_start, and event_end required")
        payload: dict = {"summary": v.event_summary, "start": {"dateTime": v.event_start}, "end": {"dateTime": v.event_end}}
        if v.event_description:
            payload["description"] = v.event_description
        r = await client.post(f"https://www.googleapis.com/calendar/v3/calendars/{v.calendar_id}/events", json=payload)
        if r.status_code != 200:
            return ToolResult.error_result(tool_id=self.tool_id, error=f"Calendar API error: {r.status_code} {r.text[:200]}")
        return ToolResult.success_result(tool_id=self.tool_id, result={"action": "create_event", "ok": True, "id": r.json().get("id")})

    async def _list_files(self, client: httpx.AsyncClient, v) -> ToolResult:
        params: dict = {"pageSize": v.max_results, "fields": "files(id,name,mimeType,size,webViewLink,modifiedTime)"}
        ...
        return ToolResult.success_result(tool_id=self.tool_id, result={"action": "list_files", "files": r.json().get("files", [])})
    ...
    async def _search_drive(self, client: httpx.AsyncClient, v) -> ToolResult:
        ...
        params = {"q": " and ".join(q_parts), "pageSize": v.max_results,
                  "fields": "files(id,name,mimeType,size,webViewLink,modifiedTime)"}
        r = await client.get("https://www.googleapis.com/drive/v3/files", params=params)
        if r.status_code != 200:
            return ToolResult.error_result(tool_id=self.tool_id, error=f"Drive API error: {r.status_code}")
        return ToolResult.success_result(tool_id=self.tool_id, result={"action": "search_drive",
            "query": v.query, "count": len(r.json().get("files", [])), "files": r.json().get("files", []),
        })
