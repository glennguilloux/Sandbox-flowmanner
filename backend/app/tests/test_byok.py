...
async def test_create_api_key():
    ...
    with patch("app.api.v1.byok.get_current_user", return_value=MOCK_USER),          patch("app.api.v1.byok.get_db") as mock_get_db,          patch("app.utils.encryption.encrypt_api_key", return_value="encrypted_key"),          patch("app.api.v1.byok.UserAPIKey") as MockUserAPIKey:
...
async def test_list_api_keys():
    ...
    with patch("app.api.v1.byok.get_current_user", return_value=MOCK_USER),          patch("app.api.v1.byok.get_db") as mock_get_db:
        ...
        mock_keys = [
            MagicMock(id=1, provider="openai", key_label="OpenAI", is_active=True, 
                      created_at=MagicMock(isoformat=lambda: "2026-04-30T12:00:00"),
                      updated_at=MagicMock(isoformat=lambda: "2026-04-30T12:00:00")),
            MagicMock(id=2, provider="anthropic", key_label="Anthropic", is_active=True,
                      created_at=MagicMock(isoformat=lambda: "2026-04-30T12:00:00"),
                      updated_at=MagicMock(isoformat=lambda: "2026-04-30T12:00:00")),
        ]
