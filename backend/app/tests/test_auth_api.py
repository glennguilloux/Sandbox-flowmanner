...
def test_register_success(test_client, mock_db_session):
    ...
    with patch("app.api.v1.auth.check_rate_limit", return_value=(True, 5, 0)), \
         ...
         patch("app.services.auth_service.create_user", return_value=MagicMock(id=1, role="user")):
            response = test_client.post("/api/auth/register", json={
                "email": "new@example.com", "password": "ValidPass123!", "username": "newuser"
            })
...
def test_register_duplicate_email(test_client, mock_db_session):
    ...
    with patch("app.api.v1.auth.check_rate_limit", return_value=(True, 5, 0)):
        response = test_client.post("/api/auth/register", json={
            "email": "existing@example.com", "password": "ValidPass123!", "username": "newuser"
        })
...
def test_login_success(test_client, mock_db_session, sample_user):
    ...
    with patch("app.api.v1.auth.check_rate_limit", return_value=(True, 5, 0)), \
         patch("app.api.v1.auth.record_failed_login", return_value={"locked": False, "attempts_remaining": 5, "lockout_seconds": 0, "progressive_delay_ms": 0}), \
...
def test_logout(test_client, sample_user):
    ...
    try:
        with patch("app.api.v1.auth.revoke_refresh_token", return_value=None):
            response = test_client.post("/api/auth/logout", json={"refresh_token": "test-refresh"}, headers={"Authorization": "Bearer test-token"})
