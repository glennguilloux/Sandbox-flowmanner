...
async def test_list_tiers():
    ...
    tiers = [
        SimpleNamespace(
            id=1, name="free", display_name="Free", description="Free tier",
            price_monthly=0.0, missions_per_day=5, missions_per_month=150,
            max_concurrent_missions=1, has_priority_support=False,
            has_api_access=False, has_custom_models=False
        ),
        SimpleNamespace(
            id=2, name="pro", display_name="Pro", description="Pro tier",
            price_monthly=29.99, missions_per_day=50, missions_per_month=1500,
            max_concurrent_missions=5, has_priority_support=True,
            has_api_access=True, has_custom_models=False
        ),
    ]
...
async def test_get_my_subscription():
    ...
    mock_db_result.scalars.return_value.first.return_value = SimpleNamespace(
        name="pro", missions_per_day=50, missions_per_month=1500,
    )
