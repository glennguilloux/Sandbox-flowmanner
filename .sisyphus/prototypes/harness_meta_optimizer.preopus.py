...
# ── Bounds for the continuous/ordinal config dims (match HarnessConfig.to_vector order) ──
# Used to normalize the surrogate's input space so no single wide-range dim
# (chunk_size) dominates the kernel distance over the narrow dims (top_k,
# cache_threshold, model index, hitl). Each entry is (lo, hi) per vector component:
#   [0] chunk_size   [1] top_k   [2] decay_half_life   [3] cache_threshold
#   [4] model index  [5] hitl_on_irreversible
BOUNDS: list[tuple[float, float]] = [
    (128.0, 1024.0),
    (1.0, 12.0),
    (1.0, 72.0),
    (0.6, 0.98),
    (0.0, float(len(KNOWN_MODELS) - 1)),
    (0.0, 1.0),
]
...
class KernelSurrogate:
    ...
    def __init__(
        ...
        self.y: list[float] = []
        # Per-dimension (lo, hi) for input normalization. Without it the wide
        # chunk_size range swamps the kernel distance over the narrow dims
        # (top_k, cache_threshold, model index), collapsing BO to 1-D.
        self.bounds = bounds
        self.lengthscale = lengthscale
        self.jitter = jitter

    def _norm(self, x: list[float]) -> list[float]:
        """Min-max normalize to [0,1] per dim so distances are scale-fair."""
        if not self.bounds:
            return list(x)
        out = []
...
class MetaOptimizer:
    ...
    def __init__(
        ...
        # Normalize the mixed-scale input space (see KernelSurrogate) so BO is
        # genuinely multi-dimensional; a single lengthscale of 200.0 over raw
        # values made chunk_size dominate and flattened the other dims.
        self.surrogate = KernelSurrogate(bounds=BOUNDS, lengthscale=0.4)
    ...
    def _propose(self, base: HarnessConfig) -> list[float]:
        if not self.surrogate.X:
            # Random init — but still respect the gate so the first observation
            # is feasible (the surrogate is only ever trained on feasible scores).
            for _ in range(200):
                cand = self._random_vector()
                if self.ev.gate.check(HarnessConfig.from_vector(cand, base))[0]:
                    return cand
            return self._random_vector()
        # Random-search the acquisition over a batch of random candidates (cheap, robust).
        # CRITICAL: filter candidates through the feasibility gate *before* scoring
        # them with EI. The gate is a hard constraint (axis 1d), and the surrogate is
        # trained only on feasible scores — otherwise EI sees the farthest infeasible
        # corner as maximally uncertain and the loop gets stuck there forever.
        best_vec, best_ei = None, -1.0
        best = self.surrogate.best_y()
        for _ in range(200):
            cand = self._random_vector()
            cfg = HarnessConfig.from_vector(cand, base)
            if not self.ev.gate.check(cfg)[0]:
                continue
            mean, sigma = self.surrogate.predict(cand)
            ei = expected_improvement(mean, sigma, best)
            if ei > best_ei:
                best_ei, best_vec = ei, cand
        # Fallback: if every random sample was infeasible (extremely unlikely),
        # return a fresh random vector and let the gate reject it next turn.
        return best_vec if best_vec is not None else self._random_vector()
    ...
    def run(self, base: HarnessConfig) -> OptResult:
        ...
        for it in range(self.max_evals):
            vec = self._propose(base)
            cfg = HarnessConfig.from_vector(vec, base)
            m = self.ev.evaluate(cfg)
            res = OptResult(cfg, m["accuracy"], m["cost"], m["latency"], m["feasible"])
            self.history.append(res)
            if not m["feasible"]:
                # Hard feasibility constraint g(θ)>=0 (axis 1d gate). Rejections
                # mean "we didn't learn," not "we can't improve" — they do NOT
                # advance the no-improvement early-stop counter (research doc §2e).
                print(
            ...
            # Select the BEST by the surrogate's own multi-objective scalar
            # (0.7*accuracy - 0.15*cost - 0.15*latency) — NOT raw accuracy — so
            # the loop honors the doc's §2c Pareto objective. A meaningful-
            # improvement window drives early stopping (no off-by-one on the
            # first feasible point: it neither falsely counts as an improvement
            # nor as a non-improvement).
