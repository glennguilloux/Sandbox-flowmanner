...
class TestContextManager:
    def test_context_manager_enter_returns_self(self):
        ...
        client._client.set_httpx_client(
            httpx.Client(transport=_mock_transport(_ok_handler(calls)), base_url="https://example.com")
        )
        with client as fm:
            assert fm is client
    ...
    def test_context_manager_exit_works(self):
        client = FlowmannerClient(base_url="https://example.com", api_key="sk-test")
        client._client.set_httpx_client(
            httpx.Client(transport=_mock_transport(lambda r: _json_response({})), base_url="https://example.com")
        )
        with client:
