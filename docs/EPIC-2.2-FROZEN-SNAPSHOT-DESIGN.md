...
> Status: **MERGED TO MAIN, NOT YET LIVE.** Build merged into `main` at `daa412fe` (merge of build commit `f8c334ea`), pushed to origin. Option A shipped as designed: no migration, no schema change. Verified on the merged main tree 2026-07-09 — E22 suite 6/6 vs live Postgres, ruff clean, memory regression 10/10 in isolation, imports OK. The current running prod image predates this merge and does **not** contain E22 yet. To go live: `bash deploy-backend.sh` (no `--migrate`). Exit audit: `.hermes/sessions/exit-audit-2026-07-09-epic22-frozen-snapshot.md`.
