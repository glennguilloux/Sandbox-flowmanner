...
> ⚠️ **SECURITY: the literal `SANDBOXD_API_TOKENS` value was previously written into this audit and committed in `241e041` → `6e17859` (8 commits on `main`, all pushed).** The token must be considered **publicly compromised** if this repo is public (the GitHub remote `glennguilloux/flowmanner` is the relevant one). **Rotate the token at `/mnt/apps/Softwares2/sandboxd/.env` IMMEDIATELY**, regardless of what happens to the untracked files. **Then** rewrite history (BFG or `git filter-repo`) on those 8 commit hashes to scrub the literal value from `main` — this requires force-push and coordination with any other clones/forks. The current redaction (`6e17859` and any follow-up) only stops the bleeding going forward; the past commits still leak.
>
> **🛑 DO NOT execute any commit in the matrix below until the rotation in step (a) is verified AND the 8 affected commits are scrubbed via BFG/filter-repo.** The matrix is paused until the security recovery is complete.
>
> **Operational caveat:** the rotation itself will surface the new token to whatever tool reads the .env (basher, agent context, etc.) — same leak vector, new value. **Recommendation:** perform the rotation via direct SSH outside the agent loop, or accept the trade-off and re-rotate after the agent is done.
...
|------|------:|-------|--------|
| `docs/HOMELAB-SERVICES-REFERENCE.md` | 309 | **Contains a live `SANDBOXD_API_TOKENS` token (format `flowmanner=<64-hex>`) at the bottom of the file, apparently pasted from a `cat` of the sandboxd .env.** The token is a real production-shaped secret (64 hex chars = 32 bytes = 256 bits). **The literal token value is intentionally NOT reproduced in this audit** — see the grep commands below for how to find it. | **Treat the token as compromised REGARDLESS of whether the file is committed** — it's been in agent conversation context. **Redact** to `SANDBOXD_API_TOKENS=<redacted — see /mnt/apps/Softwares2/sandboxd/.env>` first, **check git history** with a robust pattern that handles both equals and colon separators: `git log -p --all | grep -E 'SANDBOXD_API_TOKENS=[^[:space:]]+' | grep -E '[a-f0-9]{64}'` (catch the line, then filter for any 64-hex char sequence; the `[a-f0-9]{64}` backstop catches any 64-char hex secret regardless of name/separator format). The narrower `=[a-zA-Z0-9_]+=[a-f0-9]{64}` pattern would miss colon-separated tokens. Also check `git reflog`. **Then commit**. **Rotate the token** at `/mnt/apps/Softwares2/sandboxd/.env` (regenerate per the sandboxd control plane's auth docs) — rotating fixes the leak going forward; past commits need BFG or `git filter-repo` to clean. **Note on the rotation itself:** see the operational caveat in the security banner above. |
